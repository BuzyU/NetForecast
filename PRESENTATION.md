...
| NetForecast (World Model) | 2-Layer LSTM + Multi-Head Rollout, Focal Loss + logit calibration | **0.862** | **0.859** | **0.864** | **0.046** |

Binary malicious-vs-benign detection on a held-out real test set from CIC-IDS2017 + CIC-IDS2018, using a proper 3-way train/val/test split so checkpoint selection never touches the reported test data (`backend/artifacts/benchmark_comparison.csv`).

Per-stage F1 on the same held-out test set: Benign 0.98, Reconnaissance 0.92, **Initial Access 0.84**, Lateral Movement 0.92, C2 0.98. Initial Access was the hardest stage; it reached 0.84 through real CIC-IDS2018 web-attack data, focal loss, logit calibration, a fix to how the original CIC-IDS2017 rows were grouped into sessions, and — the biggest single step — training the stage head on the flow the live system actually labels instead of the unseen next flow (`docs/model_card.md` §5). Exfiltration isn't learnable from the 2 real examples in the test set; it's covered by a deterministic Heartbleed signature detector instead.

Any flagged session can be explained on demand, not just scored: SHAP (KernelExplainer, sampled Shapley estimates) identifies which of the 22 telemetry features pushed the model toward predicting compromise, and gradient×input attribution gives an instantaneous alternative where a SHAP pass is too slow. In live operation, a per-session adaptive EMA threshold ($\bar{p} + 2\sigma$, never below 0.5) raises the alert bar for sessions whose risk is persistently noisy, reducing repeat alerts on the same traffic. (The benchmark FPR above is measured offline at the fixed 0.5 threshold, without this adaptive layer.)
